...
function secondsToHoursFormat(seconds, text){
...
nv.models.multiBarChart = function() {
  ...
  var margin = {top: 30, right: 20, bottom: 50, left: 60}
    ...
    , tooltip = function(key, x, y, e, graph) {
          var idx = y.indexOf(".");
          var sec = y.slice(0,idx);
          var seconds = sec.split(/[\s,.]+/);
          seconds=seconds.join("");
          var value = secondsToHoursFormat(seconds);
          return '<h3>' + key + '</h3>' +
               '<p>' +  value + ' on ' + x + '</p>'
      }
    , x //can be accessed via chart.xScale()
  ...
  function chart(selection) {
    ...
    selection.each(function(data) {
      ...
      /*data.forEach(
          function(data_el){
              //console.log(data_el);
              data_el.values.forEach(
                  function(o, i, array){
                      if(o.value==null){
                         // console.log(o);
                          var idx = array.indexOf(o);
                          if(idx>-1)
                            data_el.values.splice(idx, 1);
                      }
        ...
        if (showLabels) {
          ...
          pieLabels.select(".nv-label text")
                ...
                .text(function(d, i) {
                  ...
                  var formattedValue = secondsToHoursFormat(getY(d.data));
                  var labelTypes = {
                    "key" : getX(d.data),
                    "value": getY(d.data),
