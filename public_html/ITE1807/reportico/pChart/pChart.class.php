   ...
   function drawPieLegend($XPos,$YPos,$Data,$DataDescription,$R,$G,$B)
    {
     ...
     $YOffset = 4 + $this->FontSize; $ID = 0;
     $paletteSize = count($this->Palette);
       
     foreach($Data as $Key => $Value)
      {
       $paletteKey = $ID;
       if ( $Key >= $paletteSize )
            $paletteKey = $Key %  $paletteSize;

       $Value2     = $Value[$DataDescription["Position"]];
       $Position   = imageftbbox($this->FontSize,0,$this->FontName,$Value2);
       $TextHeight = $Position[1]-$Position[7];
       $this->drawFilledRectangle($XPos+10,$YPos+$YOffset-6,$XPos+14,$YPos+$YOffset-2,$this->Palette[$paletteKey]["R"],$this->Palette[$paletteKey]["G"],$this->Palette[$paletteKey]["B"]);
   ...
   function drawBasicPieGraph($Data,$DataDescription,$XPos,$YPos,$Radius=100,$DrawLabels=PIE_NOLABEL,$R=255,$G=255,$B=255,$Decimals=0)
    {
     ...
     foreach ($PolyPlots as $Key => $Value)
      { 
       ...
       $C_GraphLo = $this->AllocateColor($this->Picture,$this->Palette[$paletteKey]["R"],$this->Palette[$paletteKey]["G"],$this->Palette[$paletteKey]["B"]);
     ...
     foreach($iValues as $Key => $Value)
      {
       ...
       $C_GraphLo = $this->AllocateColor($this->Picture,$this->Palette[$paletteKey]["R"],$this->Palette[$paletteKey]["G"],$this->Palette[$paletteKey]["B"],-20);
       ...
       for($j=0;$j<=count($aBotPlots[$Key])-4;$j=$j+2)
        $this->drawLine($aBotPlots[$Key][$j],$aBotPlots[$Key][$j+1],$aBotPlots[$Key][$j+2],$aBotPlots[$Key][$j+3],$this->Palette[$paletteKey]["R"]+$En,$this->Palette[$paletteKey]["G"]+$En,$this->Palette[$paletteKey]["B"]+$En);
      }
       ...
       foreach($iValues as $Key => $Value)
        {
         ...
         $C_GraphLo = $this->AllocateColor($this->Picture,$this->Palette[$paletteKey]["R"],$this->Palette[$paletteKey]["G"],$this->Palette[$paletteKey]["B"],-10);
         $Plots = array(); $Plot = 0;
         ...
         $this->drawAntialiasPixel($Plots[0],$Plots[1],$this->Palette[$paletteKey]["R"]+$ColorFactor,$this->Palette[$paletteKey]["G"]+$ColorFactor,$this->Palette[$paletteKey]["B"]+$ColorFactor);
         $this->drawAntialiasPixel($Plots[2],$Plots[3],$this->Palette[$paletteKey]["R"]+$ColorFactor,$this->Palette[$paletteKey]["G"]+$ColorFactor,$this->Palette[$paletteKey]["B"]+$ColorFactor);
         $this->drawAntialiasPixel($Plots[$Index-4],$Plots[$Index-3],$this->Palette[$paletteKey]["R"]+$ColorFactor,$this->Palette[$paletteKey]["G"]+$ColorFactor,$this->Palette[$paletteKey]["B"]+$ColorFactor);
        }
      }
     ...
     for($Key=count($iValues)-1;$Key>=0;$Key--)
      { 
       ...
       for($j=0;$j<=count($aTopPlots[$Key])-4;$j=$j+2)
        $this->drawLine($aTopPlots[$Key][$j],$aTopPlots[$Key][$j+1],$aTopPlots[$Key][$j+2],$aTopPlots[$Key][$j+3],$this->Palette[$paletteKey]["R"]+$En,$this->Palette[$paletteKey]["G"]+$En,$this->Palette[$paletteKey]["B"]+$En);
      }
    }
       ...
       foreach($Values as $key2 => $Value)
        {
         ...
         if(($key2 === 'UserSubTotal0' || $key2 === "ProjectSubTotal0" || $key2=="ProjectSubEstimated1")  && ($Value === "" || $Value==0))
          continue;
